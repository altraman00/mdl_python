...
class User(db.Model):
    ...
    face = db.Column(db.String(64), unique=True)  # 头像
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 注册时间
    uuid = db.Column(db.String(64), unique=True)  # 唯一标识符
...
class Movie(db.Model):
    ...
    url = db.Column(db.String(64), unique=True)  # 地址
    info = db.Column(db.Text)  # 电影简介
    logo = db.Column(db.String(64), unique=True)  # 封面
